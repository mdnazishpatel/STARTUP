...
//   ✅ SEMICOLON ADDED HERE — THIS IS CRITICAL
// server.js — Enhanced /code route for massive SaaS generation
...
app.post('/like', authenticateToken, async (req, res) => {
  try {
    ...
    const idea = await Idea.findOneAndUpdate(
      { 
        _id: new mongoose.Types.ObjectId(ideaId), 
        userId: new mongoose.Types.ObjectId(req.user.id) 
      },
      { $set: { isLiked: true } },
      { new: true }
...
app.post('/unlike', authenticateToken, async (req, res) => {
  const { ideaId } = req.body;
  const idea = await Idea.findOneAndUpdate(
    { _id: new mongoose.Types.ObjectId(ideaId), userId: new mongoose.Types.ObjectId(req.user.id) },
    { $set: { isLiked: false } },
    { new: true }
  );
  res.json({ success: !!idea, idea });
});
